  ...
  <div class="common-manage">
    <el-card>
      ...
      <el-table :data="tableData" v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="booking_count" label="预订数" width="100" v-if="showStats" />
        <el-table-column prop="total_revenue" label="总收入" width="120" v-if="showStats">
          <template #default="{ row }">
            ¥{{ row.total_revenue || 0 }}
          </template>
        </el-table-column>
        <el-table-column prop="room_occupancy" label="入住率" width="100" v-if="showStats">
          <template #default="{ row }">
            {{ row.room_occupancy || 0 }}%
          </template>
        </el-table-column>
        <el-table-column prop="view_count" label="浏览数" width="100" v-if="showStats" />
        ...
        <el-table-column label="操作" fixed="right" width="200">
          <template #default="{ row }">
            <el-button type="primary" link @click="handleEdit(row)" v-if="showEdit">
...
const currentConfig = pageConfig[routePath] || pageConfig['/daily-recommend']
...
const showStats = computed(() => currentConfig.showStats)
...
const showEdit = computed(() => currentConfig.showEdit !== false)
