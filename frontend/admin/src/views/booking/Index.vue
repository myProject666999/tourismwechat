...
const pageConfig = {
  '/booking': { 
    ...
    name: '预订', 
    api: '/admin/bookings', 
    exportApi: '/admin/export/bookings',
    showName: false, 
    showTitle: false, 
    showHotel: true, 
    showContact: true, 
...
const currentConfig = pageConfig[routePath] || pageConfig['/booking']
...
const getStatusType = (status) => {
  const types = { 0: 'warning', 1: 'success', 2: 'primary', 3: 'info' }
...
const handleExport = async () => {
  if (!currentConfig.exportApi) {
    ElMessage.warning('该模块暂不支持导出')
    return
  }
  try {
    const res = await request.get(currentConfig.exportApi, {
      responseType: 'blob'
    })
    ...
    link.href = url
    link.download = `导出数据_${new Date().toLocaleDateString()}.xlsx`
    link.click()
